...
sidebar_position: 2
title: "Summer"
description: "Buzzer-Kurs – Sounds und Melodien auf Eliobot"
---

# Summer

Die Klasse `Buzzer` steuert den passiven Summer von Eliobot. Es ermöglicht das Abspielen von Tönen, Musiknoten und vordefinierten Klängen.
...
Spielt einen Ton mit einer bestimmten Frequenz ab.
...
Spielt eine Note aus einem Frequenzwörterbuch.
...
:::tip
Verwenden Sie den Wert `0.1`, um eine Pause im Notenwörterbuch darzustellen.
:::

### `sweep(start, end, dur, steps=10, pause=0.01)`

Scannt einen Frequenzbereich von `start` bis `end` in `steps` Schritten über `dur` Sekunden.
...
Der Kurs umfasst eine Sammlung gebrauchsfertiger Sounds:
...
## Vollständiges Beispiel – Spielen einer Melodie
